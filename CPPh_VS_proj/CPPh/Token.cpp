...
Token::Token(Options opt)
{
	...
	memcpy(this->upass, opt.opt_p, strlen(opt.opt_p));
...
Token::Token()
{	
	TCHAR *defaultReader = _T("Aladdin Token JC 0");
	...
	memset(this->reader, 0, this->lreader);
	wcsncpy(this->reader, defaultReader, wcslen(defaultReader));

	memset(this->ATR, 0, this->lATR);

	this->establish_token();
}

int Token::setUpass(char *p){
	if (strlen(p) < (this->lpass - 1)){
		strncpy(this->upass, p, strlen(p));
		return 0;
	}
	else
		return 1;
...
void Token::list_containers(vector<string> &res){
	...
	while (true){
		...
		BYTE container_name_len = pbRecv[3];
